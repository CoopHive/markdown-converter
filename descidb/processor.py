...
class Processor:
    def __init__(self, authorPublicKey: str, db_manager: VectorDatabaseManager, postgres_db_manager: PostgresDBManager, metadata_file: str, ipfs_api_key: str, TokenRewarder: TokenRewarder):
        """
        ...
        for db_config in databases:
            ...
            for chunk_index, chunk in enumerate(chunked_text):
                ...
                self.db_manager.insert_document(
                    db_name=db_name, embedding=embedding, metadata=metadata, doc_id=f"{doc_id}_chunk_{chunk_index}"
                )

                self.postgres_db_manager.insert_data(db_name=db_name, data=[(
                    self.authorPublicKey, metadata.get("title", "Unknown Title"), chunk, embedding, json.dumps(metadata), False)])
